create table autuacao.classe_originaria (sig_classe varchar2(6) not null, nom_classe varchar2(100) not null, constraint pk_classe_originaria primary key (sig_classe));
...
alter table autuacao.classe_preferencia add constraint fk_classe_originaria_clpr foreign key (sig_classe) references autuacao.classe_originaria(sig_classe);
...
create table autuacao.tipo_peca (seq_tipo_documento number not null, nom_tipo_documento varchar2(100) not null, constraint pk_tipo_anexo primary key (seq_tipo_documento));

create table autuacao.peca (seq_peca number not null, dsc_peca varchar2(50) not null, seq_tipo_peca number not null, seq_documento number not null, seq_processo number not null, constraint pk_peca primary key (seq_peca));
...
create table autuacao.parte (seq_parte number not null, nom_apresentacao varchar2(50) not null, tip_polo varchar2(11) not null, seq_pessoa number, seq_processo number not null, constraint pk_parte primary key (seq_parte));
